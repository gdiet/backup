...
pub trait MountFilesystem: Send + Sync + 'static {
    ...
    /// ordinary rename (see `RENAME_NOREPLACE`'s doc comment). WinFSP
    /// already rejects a colliding no-replace rename before this method
    /// is ever called, so the caller-visible behavior is still correct;
    /// only an implementation that specifically depends on observing
    /// `no_replace = true` itself (rather than just the collision being
    /// rejected) is affected.
