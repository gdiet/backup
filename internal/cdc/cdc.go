...
// NewCDC provides a CDC chunker that produces an average chunk size of a little more than
// targetSize, where targetSize is 2 ^ targetSizeBits. The minimum chunk size is
// baseSize = targetSize / 2, the maximum chunk size is targetSize / 2 * (targetSizeBits + 1).
// Chunk limits are detected by checking whether the last N bits of the current fingerprint
// are all 0. Each baseSize bytes, one bit less is required by the chunk limit detection.
//
