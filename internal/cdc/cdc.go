...
func NewCDC(normSizeBits int) Chunker {
	...
	return &cdcChunker{
		...
		maskSmall: uint64(maxSize - 1), // e.g. 0x 3fffff  Corresponds to the default fastcdc-go 2 bit normalization
		maskLarge: uint64(minSize - 1), // e.g. 0x 03ffff  Corresponds to the default fastcdc-go 2 bit normalization
	}
}
...
outer:
	for i := 0; i < len(data); {
		startOfChunk := i - c.chunkLength

		// Skip data up to minSize
		end := startOfChunk + c.minSize
		// Handle the case that remaining data is smaller than minSize
		if end >= len(data) {
			c.chunkLength = len(data) - startOfChunk
			return chunkPositions
		}
		// Skip
		i = max(i, end)

		// Fingerprint up to normSize
		end = min(startOfChunk+c.normSize, len(data))
		for ; i < end; i++ {
			c.fingerprint = (c.fingerprint << 1) + table[data[i]]
			if (c.fingerprint & c.maskSmall) == 0 {
				i++
...
// table contains the data used for the rolling fingerprint function. It's the same table as in
// https://github.com/SaveTheRbtz/fastcdc-go v0.3.0 to achieve chunking at the same positions.
// The original fastcdc-go is licensed under the Apache License Version 2.0.
// Thanks to https://github.com/SaveTheRbtz, https://github.com/jotfs, and https://github.com/eadanfahey.
