This package provides tree, file and data reference metadata for the deduplicating file system, implemented as bbolt buckets.

### bbolt Buckets For The File System Metadata

#### General Format

- id and start are 8-byte big endian uint64 values.
- length is an 8-byte big endian int64 value.
- time is stored as an 8-byte big endian int64 unix time in milliseconds.
- hash is the 32-byte Blake3 hash.
- File system settings are persisted as key-value string pairs.

#### Buckets:

| Bucket             | Key                    | Value                   |
| ------------------ | ---------------------- | ----------------------- |
| tree entries       | id                     | dir entry or file entry |
| children           | parent id and child id | ---                     |
| data entries       | length and hash        | list of start and end   |
| free areas         | start                  | length                  |
| settings (planned) | string                 | string                  |

#### Dir Entry:

| Field     | Size (bytes) | Description          |
| --------- | ------------ | -------------------- |
| type      | 1            | entry type (value 0) |
| name      | average 23   | file name (UTF-8)    |
| **total** | 24           |                      |

#### File Entry:

| Field            | Size (bytes)    | Description            |
|------------------|-----------------|------------------------|
| type             | 1               | entry type (value 1)   |
| last modified    | 8               | unix time milliseconds |
| data reference 1 | 8 + 32 = 40     | length and hash        |
| data reference 2 | 8 + 32 = 40     | last length is < 1     |
| name             | average 23      | file name (UTF-8)      |
| **total**        | 72              |                        |

A file entry can have multiple data references to allow for content defined chunking. The end of the data references is indicated by a data reference with a length of less than 1. If negative, the sign only indicates the end of the data references, and the absolute value is the length of the last chunk.

#### Estimated Metadata Size of a Single Small File:

| Field            | Size (bytes) |
| ---------------- | ------------ |
| tree entry id    | 8            |
| file entry       | 72           |
| children         | 16           |
| data entry key   | 40           |
| data entry value | 16           |
| bbolt overhead   | 48           |
| **total**        | 200          |
