...
package object fx {
  ...
  def runAsync(f: => Unit, onException: UncaughtExceptionHandler = new UncaughtExceptionHandler {
    override def uncaughtException(t: Thread, e: Throwable): Unit = e.printStackTrace()
  }) = Future(try f catch {
    case e: Throwable => onException uncaughtException (Thread.currentThread(), e)
