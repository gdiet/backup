...
// The data store is thread safe. Backing data files are filled up to fileSize bytes. Negative
// offsets are not supported.
//
// The recommended production fileSize is 100.000.000 bytes. That way, data files can be copied
// fast (not too many files which slows down copy) while having a manageable size for all kinds
// of file systems. The recommended production storage size limit is fileSize * 10.000.000 bytes,
// i.e. 1.000 TB for fileSize = 100 MB.
//
// openFilesSoftLimit is an important performance factor for parallel access. Set it at least to
// the expected number of concurrent accesses and to not less than 5.
//
// A locking strategy is implemented to optimize for concurrent access:
//
...
func (ds *dataStore) leaseFileHandle(fileID int64, forWriting bool) (*fileHandle, error) {
	...
	dirPath, filePath := ds.dataFilePath(fileID)
	...
	if forWriting {
		if err := os.MkdirAll(dirPath, 0755); err != nil {
			ds.lock.Unlock()
			return nil, fmt.Errorf("failed to create directory %q: %v", dirPath, err)
		}
...
// dataFilePath returns the directory path and file path for a given fileID.
// Puts 100 files in one directory and 100 directories in one parent directory
// to avoid too many files in one directory. Designed to handle up to 1 million
// files, i.e. up to 100 TB for fileSize = 100 MB.
func (ds *dataStore) dataFilePath(fileID int64) (dirPath string, filePath string) {
	// Example: fileID=123456 -> dirNames="123456"
	dirNames := fmt.Sprintf("%06d", fileID)
	// Split into two directory levels for scalability
	dir1 := dirNames[:len(dirNames)-4]                  // e.g., "12"
	dir2 := dirNames[len(dirNames)-4 : len(dirNames)-2] // e.g., "34"
	dirPath = filepath.Join(ds.baseDir, dir1, dir2)
...
func (ds *dataStore) releaseFileHandle(fileID int64) {
	...
	if !exists {
		...
		return // Graceful fallback in production where assertion only logs, then continues.
	}
	...
	for id := range ds.free {
		...
		go func() {
			if err := handle.file.Sync(); err != nil {
				slog.Error("failed to sync", "file", handle.file, "error", err)
			}
