...
/// A completed chunk's [`LengthHash`] together with its raw bytes, produced by
/// [`BufferingHashingChunker`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ChunkWithBytes {
    pub length_hash: LengthHash,
    pub bytes: Vec<u8>,
}

/// Like [`HashingChunker`], but also buffers each chunk's raw bytes and returns
/// them alongside its [`LengthHash`].
///
/// `HashingChunker` feeds each completed chunk's bytes to the hasher and drops
/// them; that's enough to only detect duplicates, but a caller that also needs to
/// write a *new* (non-duplicate) chunk's bytes somewhere would otherwise have to
/// re-read the file to get them again. This type reads the file exactly once
/// regardless of hit/miss ratio, at the cost of buffering up to one chunk's worth
/// of bytes at a time (bounded by the chunker's configured max chunk size).
///
/// This can't be built by wrapping [`HashingChunker`] from the outside, since that
/// type hashes and discards each byte slice internally without exposing it - so
/// this duplicates its short slicing loop instead.
pub struct BufferingHashingChunker<H, C> {
    chunker: C,
    hasher: H,
    buffer: Vec<u8>,
}

impl<H: ChunkHasher, C: Chunker> BufferingHashingChunker<H, C> {
    pub fn new(hasher: H, chunker: C) -> Self {
        Self {
            chunker,
            hasher,
    ...
    /// Feed `data` to the chunker. Returns all chunks completed by this call,
    /// each with its own bytes (a fresh `Vec`, not shared with `data`).
    pub fn next(&mut self, data: &[u8]) -> Vec<ChunkWithBytes> {
        let mut bytes_into_chunk = self.chunker.bytes_into_chunk();
        let chunk_lengths = self.chunker.next(data);
        let mut result = Vec::new();
        let mut data = data;
    ...
    /// Flush the last incomplete chunk, if any. Resets the chunker.
    pub fn flush(&mut self) -> Option<ChunkWithBytes> {
        self.chunker.flush().map(|length| {
            let hash = self.hasher.finalize_reset();
            ChunkWithBytes {
...
mod tests {
    ...
    // --- BufferingHashingChunker tests ---

    #[test]
    fn test_buffering_hashing_chunker_basic() {
        let mut chunker = BufferingHashingChunker::new(XorHasher(0), SingleChunkChunker::default());
        assert_eq!(chunker.next(b"hello"), vec![]);
    ...
    #[test]
    fn test_buffering_hashing_chunker_multipart_buffers_across_calls() {
        // A chunk spanning multiple next() calls must still buffer all its bytes,
        // not just the bytes from the call that completed it.
        let data: Vec<u8> = (0u8..=255).collect();

        let mut chunker = BufferingHashingChunker::new(XorHasher(0), SingleChunkChunker::default());
        assert_eq!(chunker.next(&data[..100]), vec![]);
        assert_eq!(chunker.next(&data[100..200]), vec![]);
        assert_eq!(chunker.next(&data[200..]), vec![]);
        let chunk = chunker.flush().unwrap();
    ...
    #[test]
    fn test_buffering_hashing_chunker_matches_hashing_chunker_and_buffers_correct_bytes() {
        // Cross-check against HashingChunker's own (already-verified) output: same
        // lengths/hashes, plus each chunk's buffered bytes must equal the
        // corresponding slice of the original input.
        let config = CdcConfig::new(20).unwrap();
        let data = pseudo_random_data(42, 7 * 1024 * 1024);

        let mut plain = HashingChunker::new(XorHasher(0), config.chunker());
        let mut plain_results = plain.next(&data);
        plain_results.extend(plain.flush());

        let mut buffering = BufferingHashingChunker::new(XorHasher(0), config.chunker());
        let mut buffered_results = buffering.next(&data);
        buffered_results.extend(buffering.flush());

        assert_eq!(buffered_results.len(), plain_results.len());
        let mut offset = 0usize;
        for (buffered, plain) in buffered_results.iter().zip(plain_results.iter()) {
            assert_eq!(buffered.length_hash, *plain);
            let len = buffered.length_hash.length as usize;
            assert_eq!(buffered.bytes, data[offset..offset + len]);
            offset += len;
        }
