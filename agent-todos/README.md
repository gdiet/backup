...
routinely worked on from more than one environment at once (see `AGENTS.md`'s "Working Across
Windows And WSL", and in practice also plain SSH to a separate Windows machine) - an agent in one
...
- `agent-todos/done/*.md` - finished items, moved here (not deleted) once complete, with a short
  note on what was actually done and by which environment/agent. Mirrors `docs/plans/` +
  `docs/plans/implemented/`'s existing convention in this repo, for the same reason: a record of
  what happened is more useful to the next agent (in this or another environment, possibly hours
  or days later) than silence - deleting on completion would just make a different agent re-check
  or re-discover the same thing.
...
**Context**: <link to the relevant plan doc, commit, or prior discussion, if any>
