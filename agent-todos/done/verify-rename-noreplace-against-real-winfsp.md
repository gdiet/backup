...
gap explicitly: WinFSP's Windows backend is documented to emulate libfuse3's high-level API via
its `cygfuse` layer, so its `rename` callback's `flags` parameter is *expected* to use the same
...
(`mountfs/src/lib.rs`). Same finding also confirmed and documented in the sibling `rust2` project's
copy of this crate.
