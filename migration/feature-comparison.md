...
| `db-backup`/`db-restore` (metadata backup/restore) | planned | REQ-MAINTENANCE-001/002 in [`../requirements/functional/maintenance.md`](../requirements/functional/maintenance.md), not yet implemented |
| `db-compact` (metadata compaction) | planned | REQ-MAINTENANCE-003, not yet implemented |
| `del <path>` (delete a tree entry without mounting) | planned | REQ-CLI-003 in [`../requirements/functional/cli-commands.md`](../requirements/functional/cli-commands.md), not yet implemented; deliberately diverges from Scala's unconditional recursive delete - REQ-CLI-003 requires an explicit opt-in to delete a directory with live children (exact flag semantics not yet decided); also covers permanent purge of a specific soft-deleted entry (REQ-TREE-009 `[deleted]` addressing) behind an explicit `--purge` flag - REQ-CLI-004 is now folded into REQ-CLI-003 |
| `find <matcher>` (search entries by name/path pattern) | planned | REQ-QUERY-002 in [`../requirements/functional/query.md`](../requirements/functional/query.md), not yet implemented |
| `stats` / `stats <path>` (repository or path statistics) | planned | REQ-QUERY-003, not yet implemented |
