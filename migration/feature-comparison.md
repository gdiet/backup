...
|---|---|---|
| Whole-file content deduplication (MD5-based) | implemented | content-defined chunking (REQ-STORAGE-002/003), using `blake3` instead of MD5, deduplicates identical whole files as a byproduct of deduplicating their identical chunks - there is no separate whole-file chunking mode; a bounded, large target chunk size serves the same "skip most sub-file-matching overhead for unlikely-to-benefit content" motivation without an unbounded-memory code path (`crates/cli/src/settle.rs`, DESIGN-MEMORY-001 in [`../docs/design/ram-budget.md`](../docs/design/ram-budget.md)) |
...
| `db-backup`/`db-restore` (metadata backup/restore) | implemented | `crates/cli/src/db_backup.rs`, `crates/cli/src/db_restore.rs`, REQ-MAINTENANCE-001/002; backup is `VACUUM INTO` (read-only, never blocks or is blocked by a concurrent mutating operation); REQ-MAINTENANCE-007's stale-backup warning after a reclaim or compaction run is not part of this - its own precondition, the store-level compaction half of REQ-STORAGE-005, does not exist yet (see `../docs/design/stale-backup-detection.md`) |
...
| `reclaimSpace` (bulk purge of aged soft-deleted entries and space reclamation) | implemented | `crates/cli/src/reclaim.rs` (`dfs reclaim`, deliberately shortened rather than mirroring Scala's `reclaimSpace`), REQ-STORAGE-004; `--min-age-days` (defaulting to 0) selects the minimum soft-deleted age to purge |
