# Native 30 KB file read-back, pseudo-random - see ../methodology.md's "Statistical approach" and
# "Workload catalog". Reads back files file30kb-create.ps1 created, picking a pseudo-randomly
# chosen existing file within whichever range it has produced so far (run file30kb-create.ps1
...
# (`$ops` itself resets each run) - see file10mb-read.sh's own comment for the full effect this has
# at larger file sizes. Fixed to pick pseudo-randomly instead, matching dir-lookup.ps1's own
...
$root = "C:\dedupfs-perf\files30kb"
$counterFile = "$root\..\counter-files30kb.txt"
if (-not (Test-Path $counterFile)) {
    throw "no files to read yet - run file30kb-create.ps1 first"
}
