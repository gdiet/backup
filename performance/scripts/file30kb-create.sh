#!/usr/bin/env bash
# Native 30 KB file creation, spread across several directories, sequential - see
...
# Content comes from `perf-gen` (crates/perf-gen), seeded per file with the counter, so every file
# is unique and no bookkeeping is needed; generation stays outside the timed loop's own cost the
# same way the PowerShell counterpart's template-and-poke scheme does - see
# developer-todos/perf-gen-shared-content-generator.md for why plain `/dev/urandom` (this script's
# previous approach) was replaced. file30kb-read.sh reads these files.
# Tool (record this in the measurement protocol): bash redirection, content from `perf-gen`.

set -euo pipefail

size=30720
root=~/dedupfs-perf/files30kb
repo_root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

mkdir -p "$root"
for i in $(seq 0 19); do mkdir -p "$root/sub$i"; done
counter_file="$root/../counter-files30kb.txt"
