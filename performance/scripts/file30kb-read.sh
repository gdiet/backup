...
# "Workload catalog". Reads back files file30kb-create.sh created, picking a pseudo-randomly chosen
# existing file within whichever range it has produced so far (run file30kb-create.sh first - this
...
[ -f "$counter_file" ] || {
  echo "no files to read yet - run file30kb-create.sh first" >&2
  exit 1
}
