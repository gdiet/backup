...
func TestMemoryRemove(t *testing.T) {
	...
	t.Run("remove multiple areas, two of them partially", func(t *testing.T) {
		...
		if mem.areas[0].position != 0 || !reflect.DeepEqual(mem.areas[0].data, bytes{1, 2}) {
			t.Errorf("unexpected first area: got %+v", mem.areas[0])
		}
		if mem.areas[1].position != 2 || !reflect.DeepEqual(mem.areas[1].data, bytes{3, 4}) {
			t.Errorf("unexpected second area: got %+v", mem.areas[1])
		}
		if mem.areas[2].position != 8 || !reflect.DeepEqual(mem.areas[2].data, bytes{5}) {
			t.Errorf("unexpected third area: got %+v", mem.areas[2])
		}
		if mem.areas[3].position != 9 || !reflect.DeepEqual(mem.areas[3].data, bytes{6, 7}) {
