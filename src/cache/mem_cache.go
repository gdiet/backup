...
// MemCacheConfig configures the adaptive memory cache behavior
...
// DefaultMemCacheConfig returns a sensible default configuration
...
// memFile represents a file stored in memory
type memFile struct {
	data     []byte
	lastUsed time.Time
}

// MemCache implements a memory-based cache that stores data in RAM.
// It adapts to available system memory and can evict data when memory pressure increases.
//
// Thread Safety:
...
// NewMemCache creates a new memory-based cache with the given configuration
func NewMemCache(config MemCacheConfig) (*MemCache, error) {
	mc := &MemCache{
