...
class WriteAlgorithmSpec extends org.scalatest.freespec.AnyFreeSpec:
  class Writer:
  ...
  "The level 2 write algorithm used when persisting asynchronously should" - {
    "fail when the data size is less than the reserved size" in {
      intercept[IllegalArgumentException](
        Backend.writeAlgorithm(Iterator(0L -> Array[Byte](1,2,3)), Seq(DataArea(100, 110)), Writer().write)
      )
    }
    "fail when the data size is more than the reserved size" in {
      intercept[IllegalArgumentException](
        Backend.writeAlgorithm(Iterator(0L -> Array[Byte](1,2,3)), Seq(DataArea(100, 102)), Writer().write)
      )
    }
    "write the data to the areas specified, optionally split up into more chunks" in {
      val writer = Writer()
      Backend.writeAlgorithm(
