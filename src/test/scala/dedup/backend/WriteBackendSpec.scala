...
class WriteBackendSpec extends org.scalatest.freespec.AnyFreeSpec with TestFile:
  ...
  val level1: WriteBackend = dedup.backend(settings).asInstanceOf[WriteBackend]
...
//      .continually {
//        Thread.sleep(10); level1.entry("/dir/10 bytes").get.asInstanceOf[FileEntry]
//      }
...
//  "release(2) the open 10 bytes file" in assert(level1.release(tenBytesId))
  ...
  "close write backend" in level1.shutdown()
