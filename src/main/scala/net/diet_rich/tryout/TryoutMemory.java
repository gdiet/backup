...
public class TryoutMemory extends FuseStubFS {
    private class MemoryDirectory extends MemoryPath {
        ...
        protected void getattr(FileStat stat) {
            stat.st_mode.set(FileStat.S_IFDIR | 511); // 0777
        ...
        synchronized void read(Pointer buf, FuseFillDir filler) {
            for (MemoryPath p : contents) {
                if (p instanceof MemoryDirectory) // FIXME don't show files for now
    ...
    private class MemoryFile extends MemoryPath {
        ...
        @Override
        protected void getattr(FileStat stat) {
            stat.st_mode.set(FileStat.S_IFREG | 511); // 0777
    ...
    public static void main(String[] args) {
        TryoutMemory memfs = new TryoutMemory();
        try {
            String path;
