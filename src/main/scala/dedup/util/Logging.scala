...
class Slf4jLogger(name: String):
  ...
  def info (msg: => String, e: => Throwable): Unit = if (logger.isInfoEnabled)  logger.info (msg, e)
  def warn (msg: => String, e: => Throwable): Unit = if (logger.isWarnEnabled)  logger.warn (msg, e)
