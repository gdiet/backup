...
object TreeDB {
  ...
  def apply(connection: Connection) : TreeDB with TreeCacheUpdater with TreeDataUpdater = 
    new TreeDB with TreeCacheUpdater with TreeDataUpdater {
      var treeAdapters = new SynchronizedQueue[TreeCacheUpdateAdapter]
      def registerUpdateAdapter(adapter: TreeCacheUpdateAdapter) = treeAdapters += adapter

      var dataAdapters = new SynchronizedQueue[TreeDataUpdateAdapter]
      def registerUpdateAdapter(adapter: TreeDataUpdateAdapter) = dataAdapters += adapter
      ...
      override def move(id: Long, newParent: Long) : Boolean = {
        ...
        try { execUpdate(moveEntry_, newParent, id) match {
          // EVENTUALLY, the update adapters should be called from a separate thread
          case 0 => false
          case 1 => treeAdapters foreach(_ moved (id, newParent)); true
