...
class CachedDataFile(val fileOffset: Long, val dataLength: Int, val file: File) {
  ...
  def write(offset: Long, data: Bytes) : Option[Bytes] = {
    ...
    val writeOffset = (offset - fileOffset) toInt
    val writeLength = math.min(data length, fileOffset + dataLength - offset).toInt
