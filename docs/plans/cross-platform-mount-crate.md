...
- **One known gap, deliberately not solved**: there's no working *in-process* clean-shutdown call yet. `fuse_exit`/`fsp_fuse3_exit(env, fuse)` was found to crash deterministically inside `winfsp-x64.dll` (root-caused with a real stack trace via `cdb`, part of the standalone "Debugging Tools for Windows": faults in `fsp_fuse3_exit+0xd`, reading address `0x478`), and externally stopping the Windows service WinFSP registers for the mount (`Stop-Service`) hangs instead of unblocking the call. This didn't block anything: a real `cli mount` on Windows doesn't need a programmatic exit at all - Ctrl+C/closing the console/killing the process already unmounts cleanly (confirmed via `Get-Service` showing no leftover registration after killing it), matching how `run_mount` already behaves on Linux ("blocks until unmounted from another terminal"). Both this crate's own integration test (`mountfs/tests/windows_mount.rs`) and `cli`'s (`cli/tests/windows_mount.rs`) run the mount as a real child process and tear it down with `Child::kill` (`TerminateProcess`) instead - not just a test workaround, this is the actual current Windows shutdown story, documented in `MountArgs`'/`run_mount`'s doc comments in `cli/src/mount.rs`.
