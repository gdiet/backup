...
/// "`read_dir` succeeds" alone is not a valid readiness signal.
...
/// since it does not override them) `mkdir`/`create` - mirrors
...
/// delivery is not scoped to a single process, only a whole group.
///
...
/// `AllocConsole()` before spawning the helper does not fix that: the
/// artificial background console it creates does not reliably deliver
