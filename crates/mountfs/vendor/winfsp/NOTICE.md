...
They are vendored (rather than fetched at build time) because WinFSP's
*runtime* installer - which is all an end user or CI machine needs to
actually mount a filesystem - does not ship headers at all (no `inc`
...
value WinFSP's own installer writes if the DLL is not already on `PATH` -
