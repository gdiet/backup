...
//! `agent-todos/done/determine-winfsp-dispatch-pool-and-stack-size.md` needed real numbers for:
//!
...
//!
//! Windows-only in substance (the stack-size measurement is a Win32 API call) - gated behind
//! `#[cfg(target_os = "windows")]` internally, with a trivial fallback `main` elsewhere, so
//! `cargo build --workspace` still succeeds on Linux (unlike `windows_mount_spike_helper.rs`, whose
//! own content has nothing platform-specific in it and needs no such gate).
...
mod imp {
    ...
    unsafe extern "system" {
        /// Win32 API (Windows 8+): the low/high address bounds of the *calling* thread's own
        /// stack. `high - low` is that thread's actual stack size - the number this probe needs,
        /// not an assumption about what Windows' default happens to be.
    ...
    struct ProbeState {
        ...
        /// One entry per distinct dispatch thread that has run a `write()` call, keyed so a
        /// thread reused across multiple `write()` calls is only measured (and stored) once.
    ...
    /// Cheap-to-clone handle onto [`ProbeState`] - `mountfs::mount` takes its filesystem by
    /// value, so this is what actually gets moved into it, while the reporter thread keeps its
    /// own clone of the same underlying `Arc` to read the live counters from outside.
    #[derive(Clone)]
    ...
    pub fn run() {
        ...
        let mountpoint = args.next().expect(
            "usage: windows-dispatch-probe-helper <mountpoint> <results-path> \
             [delay-ms, default 150]",
        );
        let results_path =
            PathBuf::from(args.next().expect(
                "usage: windows-dispatch-probe-helper <mountpoint> <results-path> [delay-ms]",
            ));
...
fn main() {
    eprintln!(
        "windows_dispatch_probe_helper is Windows-only (measures WinFSP's own dispatch pool \
         via a Win32 API) - see crates/mountfs/src/bin/windows_dispatch_probe_helper.rs"
    );
    std::process::exit(1);
}
