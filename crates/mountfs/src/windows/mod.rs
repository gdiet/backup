...
//! binds directly against real libfuse3 (see `docs/design/mount-abstraction.md`).
//! `fuse_exit`/`fsp_fuse3_exit` is confirmed to crash deterministically
//! inside `winfsp-x64.dll`, not something this crate can fix - not needed
//! in practice, though: on Ctrl+C, WinFSP's own
...
pub fn mount<T: MountFilesystem>(fs: T, mountpoint: &Path, read_only: bool) -> io::Result<()> {
    ...
    // blank, unlike a real drive. Windows-only: this mount option is not
    // meaningful the same way on Linux, where a FUSE mount's "label" is
    // not a GUI-visible concept the way it is here.
