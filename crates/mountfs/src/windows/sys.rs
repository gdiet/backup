...
//! registry value WinFSP's installer writes if the DLL is not already on
...
/// non-`_EX` variant (`FSP_FUSE_USE_STAT_EX` is not defined).
#[repr(C)]
...
/// GCC) starts a *second* 4-byte unit for the field that does not fit
...
/// A `fuse3_operations` slot this crate does not implement yet - see
...
/// fork, so there is nothing for either to do; WinFSP's own header stubs
...
/// `LoadLibraryW("winfsp-x64.dll")` as-is first (found if it is already on
...
/// where real libfuse3 is linked at build time and cannot be "missing" at
/// this point - see `AGENTS.md`'s "Code Quality" section on when a bare
/// panic is/is not appropriate).
