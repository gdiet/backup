...
//! `docs/design/mount-abstraction.md`). This is the same API surface
//! WinFSP's `cygfuse` compatibility layer emulates on Windows, which is the
//! whole point of building against it instead of the `fuser` crate's
//! low-level, Linux-only `/dev/fuse` protocol.
//!
//! The full read-only set plus the structural read-write set
//! (`mkdir`/`create`/`unlink`/`rmdir`/`rename`/`utimens`/`chmod`/`chown`)
//! and `write`/`truncate` are given real signatures; every
...
//! platform this targets) without committing to a signature before it is
//! implemented.
...
/// A `fuse_operations` slot this crate does not implement yet. Same size
/// (one pointer) and alignment as every other slot regardless of its real
/// C signature, so leaving slots typed this way does not affect the
...
/// time, it is genuinely possible to run this binary on a system without it
...
/// section on when a bare panic is/is not appropriate).
...
/// which a hand-written binding has to pass explicitly since it cannot rely
