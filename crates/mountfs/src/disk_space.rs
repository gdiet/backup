...
//! point" approach (which is what the `sysinfo` crate's `Disks` does). That
//! approach was considered and rejected: it is a trap for a process that is
//! itself serving a mount, since enumerating every filesystem necessarily
//! includes this process's own mount point, and querying *that* one blocks
//! in the kernel until this same process's own
...
//! before allowing a save) triggered a `statfs` call through to an
//! enumerate-every-mount implementation built on `sysinfo`. A single-path
//! query has no such risk: it never looks at any filesystem's mount point
//! other than the one `path` itself resolves to, so it can never recurse
//! into a mount this process happens to be serving.
...
mod imp {
    ...
    /// `f_bsize * f_bavail`) the `sysinfo` crate itself uses, confirmed
    /// correct via a real `df -h` on a live mount; only enumerating *every*
    /// mount to find the right one carried the deadlock risk described
    /// above.
