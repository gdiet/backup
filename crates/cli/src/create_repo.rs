...
pub fn run(path: &Path, cdc_target_size_bits: u32, default_path_used: bool) {
...
mod tests {
    ...
    fn try_run_rejects_a_target_size_above_the_23_bit_ceiling() {
        let dir = unique_temp_path();
        let message = try_run(&dir, 24, false)
    ...
    fn try_run_gives_an_actionable_message_when_the_default_path_is_unusable() {
        ...
        let message = try_run(&path, 20, true).expect_err("must fail - the parent does not exist");
        assert!(
    ...
    fn try_run_surfaces_the_raw_error_when_an_explicit_path_is_unusable() {
        ...
        let message = try_run(&path, 20, false).expect_err("must fail - the parent does not exist");
        assert!(
            !message.contains("not usable"),
