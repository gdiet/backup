...
impl RepositorySettings {
    /// `cdc_target_size_bits`: content-defined chunking's target size. Not validated here -
    /// validating a user-supplied value is the caller's job, reusing `cdc::ChunkerConfig::new`
    /// directly (DESIGN-METADATA-009's "CLI validation" section) - the `repository_settings`
    /// table's own `CHECK` constraint is the last-resort backstop, not the primary check.
    ///
    ...
    /// type's concern.
    pub fn new(cdc_target_size_bits: u32, creation_time_millis: i64) -> Self {
        Self {
