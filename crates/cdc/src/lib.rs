...
impl Chunker for CdcChunker {
    fn next(&mut self, data: &[u8]) -> Vec<u64> {
        ...
        'outer: loop {
            ...
            //
            // Combining with XOR (not addition, as some Gear-hash variants do) matters here
            // too: addition's carry only flows from low bits to high bits, so a bit position
            // with no carry flowing in (e.g. bit 0 after a left-shift-then-add) ends up equal
            // to the table value's bit alone, independent of prior history - a table with that
            // bit set for every possible byte would leave that fingerprint bit permanently
            // stuck. XOR has no carries at all, so every output bit always combines one
            // shifted history bit with one fresh table bit; no bit position can become
            // independent of history this way, regardless of the table's bit distribution
            // (which is still kept balanced - see `test_table_properties` - as a second,
            // independent safeguard, not the only one).
...
/// A [`Chunker`]-adapter that emits a [`LengthHash`] for each chunk boundary the wrapped
/// [`Chunker`] finds, hashing bytes as they stream in and calling the hasher's `finalize_reset`
/// at each boundary.
...
impl<H: ChunkHasher, C: Chunker> HashingChunker<H, C> {
    ...
    /// The number of bytes accumulated so far in the current incomplete chunk, i.e. since the
    /// last chunk boundary (or since construction/reset if none yet) - delegates to the wrapped
    /// [`Chunker`].
