...
class FileSystem(repository: Repository) { import repository._
  val maybeRepositoryReadWrite = repository match { case r: RepositoryReadWrite => Some(r); case _ => None }
  def write[T](f: RepositoryReadWrite => T): T =
